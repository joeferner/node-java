...
if (process.platform === 'win32') {
  binaryFileName = 'jvm.dll';
} else if (process.platform === 'darwin') {
  binaryFileName = 'libjvm.dylib';
} else {
  binaryFileName = 'libjvm.so';
}

var dll_info_path = '../build/jvm_dll_path.json';
var jvm_dll_path = '';

if (fs.existsSync(dll_info_path)) {
  jvm_dll_path = require(dll_info_path).replace(path.delimiter, '');
  process.env.PATH += require(dll_info_path);
}

if (!fs.existsSync(path.join(jvm_dll_path, binaryFileName))) {
  require('../postInstall');
}
