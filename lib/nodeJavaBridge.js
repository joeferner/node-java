...
var reservedPropertyNames = [ 'name', 'attributes', 'caller' ];

function usableName(obj, name) {
  // Node 0.10 and 0.12 have different behavior for whether function.name is declared writable or not.
  // This test is constructed so that any unwritable property name OR the name 'name' will be altered,
  // to give us consistent behavior for the one known problem case of properties named 'name'.
  if (!isWritable(obj, name) || _.includes(reservedPropertyNames, name)) {
...
java.import = function(name) {
  ...
  for (i = 0; i < fields.length; i++) {
    ...
    if (((modifiers & MODIFIER_PUBLIC) === MODIFIER_PUBLIC)
      ...
      var jsfieldName = usableName(result, fieldName);
